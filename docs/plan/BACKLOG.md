...
| T-6 | Untested `init`/`bucket`/`info`/`list` (unit tests for `ManifestGenerator` helpers, `merge_manifests`, `truncate_desc`, `detect_shell_configs`, `update_shell_config`; `tests/cli.rs` runs bucket add/list/del, `list --all`, `info`, and idempotent `init -y` offline) | COMMIT |
