...
| IM-13 | 2026-09-23 | 2026-09-23 | P2 | `wenget config` fails when `$EDITOR` contains command-line arguments | `commands/config.rs` `detect_editor`, `run` | XS | `$VISUAL` checked first; command string split by whitespace into binary and arguments |
...
| CL-4 | 2026-09-23 | 2026-09-23 | P2 | `config.toml` template comments document incorrect default bin directory | `core/preferences.rs` `generate_default_file` | XS | Comments in generated configuration template cite `~/.local/bin` matching implementation |
