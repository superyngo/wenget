...
| OP-2 (P-2) | Cache and Installed set re-read during `update`/`add` (`add::run_with` takes the caller's `InstalledSet` and `ManifestCache`; `update_cache_with_packages` updates the in-memory cache; `update` now loads the cache 0 extra times and the Installed set once, `add` loads the cache once) | COMMIT |
