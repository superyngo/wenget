...
| CL-8 | 2026-09-23 | 2026-09-23 | P2 | No headless `Installer`: `install_packages`/`install_package` mix resolution, download, staging and terminal UI (split out of CL-1) | `commands/add.rs` `install_packages`, `install_package`; overlaps CL-6 | M | `Installer` returns structured results with UI in the command layer; needs T-3 fixture coverage first |
