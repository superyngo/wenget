...
| B-7 | 2026-09-23 | 2026-09-23 | P2 | `init` with `WENGET_ROOT` set still edits the real shell rc files (`~/.zshrc`, `~/.profile`; registry on Windows), so sandbox/test roots leak PATH lines into the user profile | `commands/init.rs` `setup_path`, `setup_path_unix` | S | With `WENGET_ROOT` set, `init` prints the PATH line instead of editing rc files/registry (or requires an explicit flag) |
