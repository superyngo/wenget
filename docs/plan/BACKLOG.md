...
| SI-7 (Q-8) | Dead items masked by `#[allow(dead_code)]` (dead helpers, enum variants, `is_exact`, `RateLimit`, non-Windows registry stubs removed; test-fixture accessors kept as `#[cfg(test)]`; remaining allows are platform- or test-scoped `cfg_attr`) | COMMIT |
