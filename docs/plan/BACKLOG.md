...
| SI-12 (A-5) | Divergent HTTP clients (`utils::http::shared_client` is the one process-wide client; `HttpClient`, downloader, and checksum probes use it with per-request timeouts; token stays `HttpClient`-only by design so asset hosts never see it) | COMMIT |
